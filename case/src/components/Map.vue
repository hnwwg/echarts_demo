...
export default {
    ...
    mounted(){
        let mapCharts = this.$echarts.init(this.$refs.map)
 var geoCoordMap = {
    '上海': [121.4648,31.2891],
    '东莞': [113.8953,22.901],
    '东营': [118.7073,37.5513],
...
let option = {
    ...
    axios.get("http://localhost:8080/MapData.json").then((res)=>{
        this.item = res.data
    }).catch((error)=>{
        window.console.log(error)
            ...
            window.setTimeout(()=>{
            // 关于图表系列内容配置的填充
            [['北京', this.item.BJData], ['上海', this.item.SHData], ['广州', this.item.GZData]].forEach(function (item, i) {
    series.push({
        name: item[0] + ' Top10',
        ...
        effect: {
            show: true, //打开特效
            period: 6, //特效的时间
            trailLength: 0.7, //特效尾迹的长度，取值0-1 值越大尾迹月长
            color: '#fff',
            symbolSize: 3  //特效标记大小
        },
        lineStyle: {
    ...
    },
    {
        ...
        data: item[1].map(function (dataItem) {
          ...
          mapCharts.on("legendselectchanged",(par)=>{
            //   window.console.log(par)
            option.title.text = par.name + "航线图"
            mapCharts.setOption(option)
          })
